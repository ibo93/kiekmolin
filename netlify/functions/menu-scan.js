// Kiek mol in — Menüscanner, normaler Weg (10 Sekunden Zeitlimit).
//
...
// Prompt wären die schlechteste Lösung: sie laufen auseinander, und ein
// Fehler ist dann mal da und mal nicht.
//
// Diese Datei ist die Rückfallebene. Der einfache Weg ohne Zeitlimit ist
// menu-scan-background.js; kann Netlify den nicht (ältere Pläne: HTTP 404),
...
exports.handler = async function (event) {
    ...
    // Nur diesen einen Abschnitt lesen (für das gleichzeitige Lesen).
    var abschnitt = body.abschnitt ? String(body.abschnitt).slice(0, 60) : '';
    // Prüfmodus: die App schickt das bereits Gelesene zurück und will wissen,
    ...
    if (body.zaehlen && images.length) {
        try {
            ...
            // Ohne Sollzahl läuft alles wie bisher -- nur ohne die Absicherung.
            return json(200, { ok: true, anzahl: null, meta: { fehler: e.message, ms: Date.now() - T0 } });
        }
    }

    // --- Nur die Überschriften ---
    // Winzige Ausgabe, ein bis zwei Sekunden. Danach weiß die App, in wie viele
        ...
        try {
            ...
            return json(200, {
                ...
                meta: {
                    ...
                    // "OK" heißt: das Modell hat nichts zu beanstanden.
                    sauber: /^\s*OK\s*$/i.test(String(pr.roh || '').trim())
                }
            });
        } catch (e) {
            // Eine gescheiterte Prüfung darf den Scan NICHT kippen -- das
            // Gelesene ist ja da. Die App behält es dann einfach ungeprüft.
            return json(200, { ok: true, korrekturen: [], fehlend: [], meta: { fehler: e.message, ms: Date.now() - T0 } });
        }
    }

    try {
        // OCR-Vorstufe nur in der ERSTEN Runde und nur, wenn dafür noch reichlich
        // Budget da ist. In einer Fortsetzung wäre sie verschenkte Zeit: das
        ...
        // noch genug Zeit ist. Früher wurde hier IMMER ein zweites Mal gefragt;
        ...
        // Duplikat-Schlüssel.
        //
        // Er enthält die GERICHTNUMMER, und das ist kein Detail: auf der echten
        // Karte von Pronto Pronto stehen "114. Roma Spezial" (Hähnchen) und
        // "115. Roma Spezial" (Rind) -- gleicher Name, gleicher Preis, zwei
        // verschiedene Gerichte. Ohne die Nummer im Schlüssel verschwindet eines
        // davon stillschweigend, und im Menü fehlt ein Gericht, das auf der
        // Karte steht.
        // Die Duplikate, gegen die dieser Schlüssel wirklich hilft (dasselbe
        // Gericht aus zwei Runden oder zwei überlappenden Bildhälften), tragen
        // dieselbe Nummer -- die werden weiterhin sauber zusammengefasst.
        // Kategorie bewusst NICHT im Schlüssel: zwei Runden kategorisieren
        ...
        // "Fleischgerichte" zurück. Wenn die App sagt "lies den Abschnitt
        ...
        // oder am Token-Limit heißt: da kommt noch was, die App fragt weiter.
        var fertig = !r.abgebrochen && r.stopReason !== 'max_tokens';

        // Anker für die nächste Runde: die letzten Namen und die zuletzt
        // gültige Überschrift. Ohne die fängt das Modell wieder oben an.
