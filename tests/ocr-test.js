...
function mk(scenario) {
  return async function (url, opts) {
    ...
    return { ok: true, json: async () => ({ candidates: [{ content: { parts: [{ text: '{"items":[{"name":"Pizza Salami","price":9.5,"category":"Pizza"}]}' }] } }] }) };
  };
}
...
(async function () {
  ...
  all &= await run('eigener Vision-Key: OCR laeuft, Text landet im Prompt',
    { GEMINI_API_KEY: 'g', GOOGLE_VISION_API_KEY: 'v' }, { vision: 'ok' },
    r => r.calls.join(',') !== 'vision,gemini' ? 'falsche Reihenfolge'
  ...
  all &= await run('kein Vision-Key: Gemini-Key wird fuer OCR probiert',
    { GEMINI_API_KEY: 'g' }, { vision: 'ok' },
    r => r.calls.join(',') === 'vision,gemini' ? true : 'OCR wurde nicht versucht');

  all &= await run('Vision 403 (API nicht aktiviert): Scan laeuft trotzdem durch',
    { GEMINI_API_KEY: 'g', GOOGLE_VISION_API_KEY: 'v' }, { vision: 'denied' },
  ...
  all &= await run('Vision meldet Bildfehler: Scan laeuft trotzdem durch',
    { GEMINI_API_KEY: 'g', GOOGLE_VISION_API_KEY: 'v' }, { vision: 'apierror' },
