...
test('Tagesplan: gewonnener Betrieb ohne CRM-Eintrag kommt auf die Liste', () => {
  const tp = require('./lib/tagesplan');
  const dienstag = new Date(2026, 7, 18, 8, 0);

  const mit = tp.baueTagesplan({
    crmLuecke: { fehlt: [{ name: 'Strandhalle', stadt: 'Norden' }] }
  }, { heute: dienstag });
  const p = mit.punkte.find((x) => x.art === 'crm-luecke');
  assert.ok(p, 'die Luecke muss auftauchen');
  assert.ok(/Strandhalle/.test(p.text), p.text);
  assert.ok(/keine Rechnung/.test(p.text), 'der Grund muss dranstehen');
  assert.strictEqual(p.sprung, '/crm/#/agentur');

  // Ohne Luecke keine Meldung - und ohne Meldung des CRM erst recht nicht.
  // Sonst stuende bei jemandem, der das CRM nie geoeffnet hat, jeder
  // gewonnene Betrieb als "fehlt" auf der Liste.
  assert.strictEqual(tp.baueTagesplan({ crmLuecke: { fehlt: [] } }, { heute: dienstag }).punkte.length, 0);
  assert.strictEqual(tp.baueTagesplan({}, { heute: dienstag }).punkte.length, 0);
});

test('CRM-Bruecke: meldet nur Nummer, Name und Ort - nichts sonst', () => {
  const b = require('./lib/crm-bruecke');

  // Das CRM schickt einen kompletten Kundendatensatz. Alles, was die
  // Agentur nicht braucht, muss hier wegfallen - VOR dem Speichern.
  const k = b.baueCrmKunden([{
    id: 'cust_1', nr: '1042', firma: 'Pizzeria La Piazza', ort: 'Emden',
    strasse: 'Hafenstr. 3', email: 'chef@lapiazza.de', telefon: '04921 123',
    iban: 'DE00 0000 0000 0000 0000 00', notizen: 'zahlt immer spaet'
  }])[0];
  assert.deepStrictEqual(Object.keys(k).sort(), ['id', 'name', 'nr', 'ort']);
  assert.strictEqual(k.name, 'Pizzeria La Piazza');
  assert.strictEqual(k.ort, 'Emden');

  // Ohne Namen ist ein Eintrag wertlos - der fliegt raus statt eine
  // namenlose Zeile in der Liste zu hinterlassen
  assert.strictEqual(b.baueCrmKunden([{ nr: '9' }, null, 'quatsch']).length, 0);

  // Derselbe Kunde doppelt gemeldet zaehlt einmal
  assert.strictEqual(b.baueCrmKunden([
    { id: 'a', name: 'Roma' }, { id: 'a', name: 'Roma' }
  ]).length, 1);

  // Kaputte Datei = leere Liste, kein Absturz
  assert.deepStrictEqual(b.leseCrmKunden('{kein json').kunden, []);
  assert.deepStrictEqual(b.leseCrmKunden('').kunden, []);
  // Beide Dateiformen werden gelesen
  assert.strictEqual(b.leseCrmKunden('[{"name":"Roma"}]').kunden.length, 1);
  assert.strictEqual(b.leseCrmKunden('{"stand":"x","kunden":[{"name":"Roma"}]}').kunden.length, 1);
});

test('CRM-Bruecke: Namensabgleich, aber kein falscher Treffer ueber Orte hinweg', () => {
  const b = require('./lib/crm-bruecke');
  const kunden = [{ id: 'c1', nr: '1042', name: 'La Piazza', ort: 'Emden' }];

  // Teilstring und Umlaute/Akzente muessen passen
  assert.ok(b.passtZu({ name: 'Pizzeria La Piazza', stadt: 'Emden' }, kunden[0]));
  assert.ok(b.passtZu({ name: 'La Piazza', stadt: '' }, kunden[0]), 'ohne Ort trotzdem Treffer');
  assert.ok(b.passtZu({ name: 'Café Löwe' }, { name: 'Cafe Loewe' }), 'Akzente und Umlaute');

  // DAS ist der gefaehrliche Fall: "Zur Muehle" gibt es mehrfach. Ein
  // falsch verheirateter Kunde waere schlimmer als gar kein Treffer.
  assert.strictEqual(
    b.passtZu({ name: 'Zur Muehle', stadt: 'Aurich' }, { name: 'Zur Mühle', ort: 'Norden' }),
    false, 'gleicher Name, anderer Ort: kein Treffer');

  // Zu kurze Teilstrings duerfen nicht greifen ("Am" in "Amrum")
  assert.strictEqual(b.passtZu({ name: 'Am' }, { name: 'Amrum Stuben' }), false);

  const liste = [
    { name: 'Pizzeria La Piazza', stadt: 'Emden', stufe: 'kontaktiert', erledigt: false },
    { name: 'Strandhalle', stadt: 'Norden', stufe: 'neu', erledigt: false }
  ];
  const v = b.verknuepfe(liste, kunden);
  assert.strictEqual(v[0].crm.nr, '1042');
  assert.strictEqual(v[1].crm, null);
  assert.strictEqual(liste[0].crm, undefined, 'das Original bleibt unangetastet');
});

test('CRM-Bruecke: die zwei Luecken, die Geld kosten', () => {
  const b = require('./lib/crm-bruecke');
  const kunden = [{ id: 'c1', nr: '1042', name: 'La Piazza', ort: 'Emden' }];
  const liste = [
    // gewonnen, aber nicht im CRM -> kann keine Rechnung bekommen
    { name: 'Strandhalle', stadt: 'Norden', stufe: 'kunde', erledigt: true, istKunde: true, schluessel: 'strandhalle--norden' },
    // CRM-Kunde, steht aber noch als offener Interessent -> nicht kalt anrufen
    { name: 'La Piazza', stadt: 'Emden', stufe: 'neu', erledigt: false, schluessel: 'la-piazza--emden' }
  ];
  const l = b.luecken(liste, kunden);
  assert.strictEqual(l.fehltImCrm.length, 1);
  assert.strictEqual(l.fehltImCrm[0].name, 'Strandhalle');
  assert.strictEqual(l.nurImCrm.length, 1);
  assert.strictEqual(l.nurImCrm[0].kundennr, '1042');

  // Der Satz nennt beide Faelle und endet sauber
  const satz = b.satzFuerLuecken(l);
  assert.ok(/keine Rechnung/.test(satz), satz);
  assert.ok(/Pipeline/.test(satz), satz);
  assert.ok(satz.endsWith('.'), satz);

  // Nichts offen: kein Satz. Eine Meldung "alles gut" liest sonst niemand.
  assert.strictEqual(b.satzFuerLuecken({ fehltImCrm: [], nurImCrm: [] }), '');
  assert.strictEqual(b.satzFuerLuecken(null), '');
});
